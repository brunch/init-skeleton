...
const initSkeleton = (alias, options) => {
  ...
  if (alias == null || alias === '.' && rootPath === cwd) {
    printBanner(options.commandName);

    const error = new Error('You should specify a skeleton');
  ...
  const clone = () => mkdirp(cacheDir).then(() => {
    ...
    if (exists(repoDir)) {
      ...
      return sh({cwd: repoDir})`git pull origin master`.then(() => {
    ...
    return sh`git clone ${url} ${repoDir}`.then(() => {
